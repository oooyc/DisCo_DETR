...
class DisCoDETR(nn.Module):
    ...
    def __init__(self,
        ...
        if multi_scale_features:
            assert transformer['type'] == 'DeformableTR' or 'RTDETR'
        ...
        self.same_randomness = same_randomness
        ...
        # additional
        self.transformer.decoder.bbox_embed = self.pred_head.bbox_embed
        self.transformer.decoder.class_embed = self.pred_head.class_embed
    ...
    def extract_reference_points(self, ori_idx, ref, name=None):
        ...
        for  batch_idx, (query_indices, _) in enumerate(last_layer_indices_v1):
    ...
    def forward_train(self, img: NestedTensor, box: List[Tensor], img_size: Tensor, crop: Tensor, name):
        ...
        if self.box_disturbance > 0 and self.same_randomness is False:
            ...
            disturbed_box_v1, disturbed_box_v2 = add_randomness_on_multiview_box(box_v1, box_v2, img_size, ratio=self.box_disturbance, same_randomness=self.same_randomness)
        ...
        target_boxes_v1 = []
        target_boxes_v2 = []
        for i, _ in enumerate(box_v1):
            target_boxes_v1.append(box_ops.box_xyxy_to_cxcywh(box_v1[i]))
            target_boxes_v2.append(box_ops.box_xyxy_to_cxcywh(box_v2[i]))
        target_boxes_v1 = torch.stack(target_boxes_v1, dim=0)
        target_boxes_v2 = torch.stack(target_boxes_v2, dim=0)
        h, w = img_size[:, 0].view(-1, 1), img_size[:, 1].view(-1, 1)
        ...
        hs_v1, mem_v2, ori_idx_v1, ref_v1 = self.transformer(src_v2_proj, mask_v2, query_embed, pos_v2, patch_v1, target_boxes_v2)  # ref is (init, inter), used for DeformableDETR only
        hs_v2, mem_v1, ori_idx_v2, ref_v2 = self.transformer(src_v1_proj, mask_v1, query_embed, pos_v1, patch_v2, target_boxes_v1)
        ...
        with torch.no_grad():
            if self.tr_class == 'DeformableTR':
                result_v1, match_dict_v1 = self.extract_reference_points(ori_idx_v1, ref_v1[1][-1], name)
                result_v2, match_dict_v2 = self.extract_reference_points(ori_idx_v2, ref_v2[1][-1], name)
            elif self.tr_class == 'ConditionalTR':
                result_v1, match_dict_v1 = self.extract_reference_points(ori_idx_v1, ref_v1, name)
                result_v2, match_dict_v2 = self.extract_reference_points(ori_idx_v2, ref_v2, name)
        ...
        # 这里的indices_v1可以不用了
        # 处理一下得到图片和patch的字典
        loss_v1 = self.pred_head(hs_v1, ref_v1, torch.stack(box_v2), img_size, recon_v1_gt, ori_idx_v1, ori_idx_v2=ori_idx_v2, hs_v2=hs_v2)
        loss_v2 = self.pred_head(hs_v2, ref_v2, torch.stack(box_v1), img_size, recon_v2_gt, ori_idx_v2)
        losses.update({k: (loss_v1[k] * 0.5 + loss_v2[k] * 0.5 if 'contrast' not in k else loss_v1[k]) for k in loss_v1.keys()})
