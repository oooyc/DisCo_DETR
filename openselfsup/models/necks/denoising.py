...
from openselfsup.utils.box_ops import box_cxcywh_to_xyxy, box_xyxy_to_cxcywh
...
def get_contrastive_denoising_training_group(targets,
    ...
    # gt_mask: [bs, max_gt_num * 2 * num_group]
    positive_gt_mask = positive_gt_mask.squeeze(-1) * pad_gt_mask
    dn_positive_idx = torch.nonzero(positive_gt_mask)[:, 1]
    # torch.nonzero 返回所有非零元素的索引
    # 返回的形状为 [N, 2]，其中：
    # - N 是非零元素的数量
    # - 2 表示两个维度的索引（batch_idx, position_idx）

    # [:, 1] 只取第二个维度的索引（position_idx）
    # 最终得到一个一维张量，包含所有正样本的位置索引
    dn_positive_idx = torch.split(dn_positive_idx, [n * num_group for n in num_gts])
    # num_gts 是一个列表，包含每个batch中实际的ground truth数量
    # [n * num_group for n in num_gts] 计算每个样本应该分配的正样本数量

    # 例子：
    # 假设 batch_size = 2
    # num_gts = [3, 2]  # 第一个样本有3个GT，第二个样本有2个GT
    # num_group = 2
    # 则分割大小为 [6, 4]  # [3*2, 2*2]
    ...
    # num_denoising = int(max_gt_num * 2 * num_group)
    # 使用之前定义的embedding层将类别索引转换为隐藏向量
    # 输入: [bs, num_denoising] 的整数索引
    # 输出: [bs, num_denoising, hidden_dim] 的嵌入向量
