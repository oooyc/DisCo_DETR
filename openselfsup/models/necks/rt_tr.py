...
class RTDETR(nn.Module):
    ...
    def __init__(self,
                    ...
                    # eval_spatial_size: Any | None = None,
                    eval_idx: int = -1,
                    eps: float = 0.01,
                    aux_loss: bool = True,
                    #--------------------decoder------------------------
        ...
        self.decoder = RTDETRTransformer(num_classes, hidden_dim, num_queries, position_embed_type, feat_channels, feat_strides, num_levels, 
                                         num_decoder_points, nhead, num_decoder_layers, dim_feedforward, dropout, activation, num_denoising, 
                                         label_noise_ratio, box_noise_scale, learnt_init_query, eval_spatial_size, eval_idx, eps, aux_loss)
        self.encoder = HybridEncoder(in_channels, feat_strides, hidden_dim, nhead, dim_feedforward, dropout, enc_act, 
                                     use_encoder_idx, num_encoder_layers, pe_temperature, expansion, depth_mult, act, eval_spatial_size)
        self.multi_scale = multi_scale
        
    def forward(self, x, patch, target_boxes=None, targets=None):
        ...
        # 这里需要分开
        x = self.encoder(x)        
        memory, output, x, ini_ref_points = self.decoder(x, patch, target_boxes, targets)

        return output, memory, x, ini_ref_points
...
class TransformerDecoder(nn.Module):
    ...
    def dist_based_ordered_patch_feature(self, patch_feature, reference_points, tgt_boxes, my_pred_out):
        ...
        patch_boxes = tgt_boxes.view(batch_size, num_patches_per_group, -1)
        ...
        return batch_indices, query_indices, patch_indices, ori_idx
    ...
    def global_optimal_matching(self, patch_boxes, ref_boxes, batch_size, num_patches_per_group, num_queries_per_group, num_matches, my_pred_out):
        ...
        cost_giou = 1 - torch.diag(box_ops.generalized_box_iou(box_ops.box_cxcywh_to_xyxy(ref_boxes), box_ops.box_cxcywh_to_xyxy(ptach_boxes)))

        # # Final cost matrix
        C = 5 * cost_bbox + 2 * cost_class + 2 * cost_giou

        C = C.view(batch_size, num_queries_per_group, -1).cpu()

        sizes = [int(num_patches_per_group) for _ in range(batch_size)]
    ...
    def forward(self,
                ...
                enc_topk_boxes=None,):
        output = tgt
        # tgt包含了denosing的class embed和encoder中topk分类置信的特征
        ...
        if patch_feature is not None:
            with torch.no_grad():
                batch_indices, query_indices, patch_indices, ori_idx = self.dist_based_ordered_patch_feature(patch_feature, enc_topk_boxes, target_boxes, enc_topk_logits)
                # if self.dist_use:
            tgt1 = torch.zeros(ref_points_detach.shape[0], ref_points_detach.shape[1], patch_feature.shape[-1], device=patch_feature.device)
            tgt1[batch_indices[:, None], query_indices, :] = patch_feature[batch_indices[:, None], patch_indices, :]
            output = output + tgt1
        ini_ref_points = enc_topk_boxes.detach()
        for i, layer in enumerate(self.layers):
        ...
        return torch.stack(intermediate_outputs), torch.stack(dec_out_bboxes), torch.stack(dec_out_logits), ori_idx, ini_ref_points
...
class RTDETRTransformer(nn.Module):
    ...
    def _get_decoder_input(self,
        ...
        # print(denoising_bbox_unact.shape) # torch.Size([4, 196, 4])
    ...
    def forward(self, feats, target_boxes=None, targets=None):

        # input projection and embedding
        (memory, spatial_shapes, level_start_index) = self._get_encoder_input(feats)
        # memory: [b, l, c]
        # spatial_shpaes: [num_levels, 2]
        ...
        # print(denoising_class.shape) # torch.Size([4, 196, 256])
        # print(denoising_bbox_unact.shape) # torch.Size([4, 196, 4])
        ...
        # decoder
        # 没有使用memory mask？？？？？？？？
        output, out_bboxes, out_logits, ori_idx, ini_ref_points = self.decoder(
            target,
        ...
        if self.training and self.aux_loss:
            ...
            out['aux_outputs'][-1]['ori_idx'] = ori_idx
            
            if self.training and dn_meta is not None:
                out['dn_aux_outputs'] = self._set_aux_loss(dn_out_logits, dn_out_bboxes)
                out['dn_meta'] = dn_meta

        return memory, output, out, ini_ref_points
