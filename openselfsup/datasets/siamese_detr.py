...
class SiameseDETRDataset(Dataset):
    ...
    def __init__(self, 
        ...
        self.two_view_crop_transform = PIPELINES.get('TwoViewRandomResizedCrop')(
            scale=(0.5, 1.), iou=self.iou, views_bbox_stochastic=self.views_bbox_stochastic, my_transform=self.my_transform)
    ...
    def __getitem__(self, index):
        try:
            sample, target, name = self.data_source.get_sample(index)
        ...
        # 随机裁剪的crop（05-1）内部生成一个相同aspect ratio和给定iou（0.5）下的的intersection
        # 同时在intersectino的基础上生成两个box，crop左上和intersection右下，同时左上内收、crop右下和intersection左上，同时右下内收
        # q和k分别表示裁剪两个box得到的resize到outer box大小的图片
        # intersectionv1表示在q视图中的intersection
        # intersectionv2表示在k视图中的intersection
    ...
    def gen_boxes(self, 
        ...
        if self.gen_box_method == 'random':
            ...
            anchors_q, anchors_k, anchors_labels = self.clip_and_remove_out_box(
                ...
                intersection_v1,
                intersection_raw)
            # anchors_q，就是unsup_box在resize后的q图中的坐标
            # anchors_k，就是unsup_box在resize后的k图中的坐标
            # anchors labels是注明anchors_k中的有效坐标
