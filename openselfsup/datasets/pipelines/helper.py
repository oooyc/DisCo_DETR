...
try:
    from torchvision.transforms.transforms import get_image_size
except:
...
def gen_two_boxes_in_outerbox_with_iou(outer_box, iou=0.5, views_bbox_stochastic=True, my_transform=False):
    """
    ...
    if my_transform is False:
        box1, box2 = [outer_x1, outer_y1, *inter_bottom_right], [*inter_top_left, outer_x2, outer_y2]  # \，取左上和右下

    if my_transform is True:
        box = [outer_x1, outer_y1, outer_x2, outer_y2]

    if views_bbox_stochastic and my_transform is False:
        ...
        box2[2] -= x_delta
        box2[3] -= y_delta
    elif views_bbox_stochastic and my_transform:
        # 左上角的box，左上角向内收缩， 单调降低union，等价于提升IoU
        delta_wmax = inter_top_left[0] - outer_x1
        delta_hmax = inter_top_left[1] - outer_y1
