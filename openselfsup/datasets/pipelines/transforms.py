...
class TwoViewRandomResizedCrop(object):
    ...
    """
    # scale=(0.5, 1.), iou=0.5, views_bbox_stochastic=true
    def __init__(self,
                 scale=(0.08, 1.0),
                 ratio=(3. / 4., 4. / 3.), 
    ...
    def __call__(self, img1, img2):
        ...
        # 在给定图片上依照给定宽高比随机裁剪得到outer box
        outer_box = (j, i, j + w, i + h)
        box1, box2, intersection = gen_two_boxes_in_outerbox_with_iou(
            outer_box=outer_box, iou=self.iou, views_bbox_stochastic=self.views_bbox_stochastic, my_transform=self.my_transform)
        # 现在outer box的中心生成一个相同aspect ratio的矩形，box1左上，box2右下
        SIZE = (h, w)
